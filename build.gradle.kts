...
/**
 * Android build mode (for PojavLauncher / ZalithLauncher style launchers).
 *
 * Enabled with `-Plb.android=true` or the `LB_ANDROID_BUILD` environment variable.
 * Excludes the desktop-only JCEF/MCEF (Chromium) runtime from the packaged JAR;
 * the code already guards all MCEF usages at runtime, so the mod still works on
 * desktop when built with the default settings.
 */
val androidBuild: Boolean = providers.gradleProperty("lb.android").orNull == "true" ||
    System.getenv("LB_ANDROID_BUILD") != null
...
dependencies {
    ...
    // JCEF Support (compile-time on every platform, only packaged on desktop builds)
    api(libs.mcef)
    if (!androidBuild) {
        include(libs.mcef)
    }
