...
class CefBrowserBackend : BrowserBackend, EventListener {
    ...
    override fun makeDependenciesAvailable(taskManager: TaskManager, whenAvailable: () -> Unit) {
        // JCEF (Chromium) has no Android build; skip it gracefully instead of
        // throwing JcefIsntCompatible() which would kill the whole client.
        if (Platform.IS_ANDROID) {
            logger.warn("JCEF/MCEF is not supported on Android; skipping browser initialization.")
            return
        }
