...
object ModuleSolsticeArraylist : ClientModule(
    ...
    /** 原版 drawShadowRectDense: 多层向外扩散的半透明圆角近似发光 */
    private fun GuiGraphicsExtractor.drawGlowRect(
        x1: Float, y1: Float, x2: Float, y2: Float,
        color: Color4b, radius: Float, density: Int,
    ) {
        if (density <= 1) {
            drawRoundedRect(x1 - radius, y1 - radius, x2 + radius, y2 + radius, radius, color)
            return
        }
        for (i in 0 until density) {
            val t = i / (density - 1).toFloat()
            val r = radius * (0.25f + 0.75f * t)
            val a = color.a * (0.5f + 0.5f * (1f - t))
            drawRoundedRect(x1 - r, y1 - r, x2 + r, y2 + r, r, color.alpha(a.roundToInt().coerceIn(0, 255)))
        }
    }

    /** 原版 drawShadowCircleDense */
    private fun GuiGraphicsExtractor.drawGlowCircle(
        ...
        for (i in 0 until density) {
            ...
            val r = radius * (0.25f + 0.75f * t)
            drawRoundedRect(cx - r, cy - r, cx + r, cy + r, r, color)
        }
    }
    ...
    private fun renderModules(ctx: GuiGraphicsExtractor, dt: Float) {
        ...
        val modules = ModuleManager.getModules().filter { it.enabled && !it.hidden }
        if (modules.isEmpty()) {
            entryRects.clear()
            return
        }
        ...
        for (mod in sorted) {
            ...
            if (glow && glowStrength > 0f) {
                val glowR = glowStrength * 100f * anim
                val glowColor = color.alpha((0.83f * 255 * anim).roundToInt().coerceIn(0, 255))
