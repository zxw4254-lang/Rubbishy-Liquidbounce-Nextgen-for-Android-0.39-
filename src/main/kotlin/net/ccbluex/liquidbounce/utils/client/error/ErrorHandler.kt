...
class ErrorHandler private constructor(
    ...
    fun buildAndShowMessage(): Boolean {
        ...
        return when {
            Platform.IS_ANDROID -> {
                // TinyFileDialogs is unavailable on Android; log the error
                // instead and (if requested) open the GitHub issue page via
                // an Android intent.
                logger.error("$title has encountered an error!\n$message")
                needToReport
            }
