...
object HttpClient {
    ...
    private val defaultClient = OkHttpClient.Builder()
        ...
        .build().also {
            // The MCEF/JCEF classes are desktop-only (no Android build). The
            // browser backend is disabled on Android, so we must not touch
            // MCEF there at all.
     ...
     *
     * This replaces the MCEF-provided `OkHttpProgressInterceptor.ProgressListener`
     * in the public API so that [HttpClient] itself does not depend on the
     * desktop-only JCEF/MCEF classes (on Android builds MCEF is not packaged).
     */
    ...
    /**
     * OkHttp interceptor that reports download progress.
     *
     * Used on Android, where the MCEF-provided progress interceptor is
     * unavailable. Purely built on the standard okhttp/okio APIs.
     */
    private class ClientProgressInterceptor(
                ...
                override fun source(): BufferedSource {
                    ...
                    return object : ForwardingSource(source) {
                        override fun read(sink: Buffer, byteCount: Long): Long {
                            ...
                            if (read != -1L) {
                                bytesRead += read
                                progressListener.onProgress(bytesRead, contentLength, false)
                            } else {
                                progressListener.onProgress(bytesRead, contentLength, true)
                            }
                            return read
                        }
                    }.buffer()
    ...
    suspend fun request(
        ...
        return if (progressListener == null) {
            ...
            // Android: MCEF classes are not available, use our own interceptor.
            client.newBuilder()
                .addNetworkInterceptor(ClientProgressInterceptor(progressListener))
                .build()
                .newCall(request).executeAsync()
        } else {
            client.newBuilder()
                .addNetworkInterceptor(
                    OkHttpProgressInterceptor(
                        OkHttpProgressInterceptor.ProgressListener { bytesRead, contentLength, done ->
                            progressListener.onProgress(bytesRead, contentLength, done)
                        }
                    )
                )
                .build()
