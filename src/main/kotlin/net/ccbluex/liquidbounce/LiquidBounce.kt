    ...
    private suspend fun prepareGuiStage(
        ...
        taskManager = TaskManager(ioScope).apply {
            ...
            launch("Deep Learning") { task ->
                if (Platform.IS_ANDROID && !DeepLearningEngine.forceEnabled) {
                    // DJL PyTorch native libraries are unreliable on Android and depend on
                    // desktop-only MCEF classes; skip the whole initialization by default.
                    task.subTasks.clear()
                    DeepLearningEngine.markUnavailable()
                    logger.info(
                        "Deep learning is disabled on Android. " +
                            "Set LB_DL_FORCE=true (or the 'net.ccbluex.liquidbounce.deeplearning.force' " +
    ...
    private val startHandler = handler<ClientStartEvent> {
        runCatching {
            ...
            if (Platform.IS_ANDROID) {
                logger.info("Running on an Android runtime (PojavLauncher/ZalithLauncher detected). " +
                    "Browser-based UI is disabled; use chat commands for configuration.")
            }
