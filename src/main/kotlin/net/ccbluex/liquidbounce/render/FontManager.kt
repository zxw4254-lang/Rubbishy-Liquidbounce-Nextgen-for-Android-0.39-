...
object FontManager {
    ...
    private val COMMON_FONT by AsyncLazy {
        runCatching {
            when {
                Platform.IS_ANDROID -> systemFont(firstAvailableFont("Roboto", "Noto Sans", "SansSerif"))
                Util.getPlatform() == WINDOWS -> systemFont("Segoe UI")
                Util.getPlatform() == OSX -> systemFont("Helvetica")
                Util.getPlatform() == LINUX -> systemFont("DejaVu Sans")
    ...
    private val CJK_FONT by AsyncLazy {
        runCatching {
            when {
                Platform.IS_ANDROID -> systemFont(
                    firstAvailableFont("Noto Sans CJK SC", "Noto Sans CJK", "Noto Serif CJK SC", "SansSerif")
                )
                Util.getPlatform() == WINDOWS -> systemFont("Microsoft YaHei")
                Util.getPlatform() == OSX -> systemFont("PingFang SC")
                Util.getPlatform() == LINUX -> systemFont("Noto Sans CJK")
    ...
    /**
     * Returns the first font family that is actually registered on this system,
     * falling back to the logical "SansSerif" font which is always available.
     *
     * Physical font availability differs a lot between desktop systems and
     * Android runtimes (PojavLauncher/ZalithLauncher register the Android
     * system fonts instead of the usual desktop ones).
     */
    private fun firstAvailableFont(vararg names: String): String =
        names.firstOrNull { name ->
            runCatching { Font(name, Font.PLAIN, 12).family.equals(name, ignoreCase = true) }
                .getOrDefault(false)
        } ?: "SansSerif"
